...
const {cartItems,cartLength,totalPrice}=storeToRefs(cartStore)
let counter=ref(1)
...
<div class="offcanvas offcanvas-end overflow-y-auto" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel" style="width: 500px;">
  ...
  <div class="offcanvas-body pt-0 d-flex flex-column">
    <p class="fs-3">{{cartLength}} items</p>

    <div v-if="!cartLength" class="d-flex flex-column">
      <p class="fs-3 my-4" style="font-weight: 500;">free shipping for all orders over {{totalPrice.toFixed(2)}}!</p>
      <p class="fs-3 my-5 text-center">your cart is empty</p>
      <button type="button" class="btn btn-outline-secondary rounded-pill shopping-btn my-5 text-capitalize">continue shopping</button>
    </div>

    <div v-if="cartLength" class="d-flex flex-column">
      <!-- progress bar -->
      <div class="progress-bar position-relative">
      <div class="progress mt-3" role="progressbar" aria-label="Danger striped example" aria-valuenow="100" aria-valuemin="0" :aria-valuemax="totalPrice" style="height: 13px; border-radius: 9px;">
        <svg class="icon-shipping-truck position-absolute" width="30px" height="30px" viewBox="0 0 40.55 24" fill="#dc3545" style="top:-7px;left:calc(20% - 30px);">
      ...
      <p class="fs-3 my-3 align-self-start" style="font-weight: 500;">only &#x24;{{totalPrice.toFixed(2)}} away from free shipping</p>
    </div>
      ...
      <div class="card row-cols-2 d-flex flex-row border-0">
        ...
        <div class="card-body col-6 position-relative d-flex justify-content-between py-5">
          ...
          <div class="cart-product-count my-3 mx-auto">
              <!-- <button type="button" class="btn fs-5" @click="counter>1?product['quantity']--:false"> - </button> -->
              <button type="button" class="btn fs-5" @click="addItem(product,(product['quantity']-1))"> - </button>
              <input type="number" class="border-0 fs-5 text-center" v-model="product['quantity']"  disabled min="1" :max="product['stock']">
            ...
            <div class="sub-total d-flex justify-content-between">
              ...
              <strong class="fs-5"> &#x24;{{ counter * (Math.ceil((product['price'] - ( product['price'] * (product['discountPercentage'] / 100) )))) }}</strong>
            </div>
        </div>
      </div>
    ...
    <button type="button" class="btn btn-primary text-white rounded-pill shopping-btn my-4 text-capitalize" style="background-color: #6ca7ea;">checkout</button>
